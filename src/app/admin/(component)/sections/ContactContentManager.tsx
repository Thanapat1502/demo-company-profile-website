...
interface ContactContentFormData {
  heroImages: File[];
}
...
export const ContactContentManager: React.FC<ContactContentManagerProps> = ({
  onSave,
  loading = false,
}) => {
  const [existingHeroImages, setExistingHeroImages] = useState<string[]>([]);

  const { control, handleSubmit, reset } = useForm<ContactContentFormData>({
    defaultValues: {
      heroImages: [],
    },
  });
  ...
  const loadHeroSection = async () => {
    try {
      const response = await fetch(`/api/hero?id=CONTACT`);
  ...
  const onSubmit = async (data: ContactContentFormData) => {
    try {
      ...
      heroFormData.append("id", "CONTACT");
      heroFormData.append(
        "existing_images",
