...
export function generateOrganizationSchema(locale: "th" | "en") {
  return {
    ...
    "hasOfferCatalog": {
      "@type": "OfferCatalog",
      "name": locale === 'th' ? "บริการก่อสร้างและวิศวกรรม" : "Construction and Engineering Services",
      "itemListElement": [
        {
          "@type": "Offer",
          "itemOffered": {
            "@type": "Service",
            "name": locale === 'th' ? "ก่อสร้างสถานีบริการน้ำมัน" : "Gas Station Construction",
            "description": locale === 'th' 
...
export function generateWebPageSchema(config: StructuredDataConfig) {
  const { locale, page, title, description, images = [], publishedTime, modifiedTime, author, breadcrumbs = [] } = config;
      ...
      "itemListElement": [
        {
          ...
          "itemOffered": {
            ...
            "description": locale === 'th' ? "บริการออกแบบสถานีบริการน้ำมัน" : "Gas station design services"
          }
        },
        {
          "@type": "Offer",
          "itemOffered": {
            "@type": "Service", 
            "name": locale === 'th' ? "ก่อสร้างสถานีบริการ" : "Gas Station Construction",
            "description": locale === 'th' ? "บริการก่อสร้างสถานีบริการน้ำมัน" : "Gas station construction services"
          }
        },
        {
          "@type": "Offer",
          "itemOffered": {
            ...
            "description": locale === 'th' ? "บริการบำรุงรักษาสถานีบริการน้ำมัน" : "Gas station maintenance services"
          }
        }
      ]
    }
...
export function generateArticleSchema(config: StructuredDataConfig & { 
  headline: string;
  articleBody?: string;
  wordCount?: number;
  tags?: string[];
}) {
  const { locale, title, description, images = [], publishedTime, modifiedTime, author, headline, articleBody, wordCount, tags = [] } = config;
  ...
  return {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `${baseUrl}/${locale}/news-events/${title?.toLowerCase().replace(/\s+/g, '-')}`
    },
